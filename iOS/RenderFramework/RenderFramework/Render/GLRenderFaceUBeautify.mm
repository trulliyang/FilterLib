...
@interface GLRenderFaceUBeautify() {
@public
    GLuint programID;
    GLuint textureID;
    GLuint vtxBufID;
...
@implementation GLRenderFaceUBeautify
...
- (void)updateUniform
{
    if (useFaceU) {
        smooth = 1.0f;
        glUniform1f(glGetUniformLocation(programID, "softenStrength"), smooth);
        glUniform2f(glGetUniformLocation(programID, "imageSize"), width, height);
    } else {
...
- (void)prepareRender
{
    ...
    if (0 == textureID) {
        NSLog(@"GLRenderFaceUBeautify do prepareRender no texture to render");
        return;
    }
...
- (void)renderFaceU
{
    ...
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, textureID);
    glUniform1i(glGetUniformLocation(programID, "aTexture0"), 0);
    ...
    //顶点流
    glBindBuffer(GL_ARRAY_BUFFER, vtxBufID);
    glEnableVertexAttribArray(glGetAttribLocation(programID, "aPosition"));
    glVertexAttribPointer(glGetAttribLocation(programID, "aPosition"), 2, GL_FLOAT, GL_FALSE, 0, NULL);
    
    //纹理坐标流
    glBindBuffer(GL_ARRAY_BUFFER, texBufID);
    glEnableVertexAttribArray(glGetAttribLocation(programID, "aTexcoord0"));
    glVertexAttribPointer(glGetAttribLocation(programID, "aTexcoord0"), 2, GL_FLOAT,GL_FALSE, 0, NULL);
    ...
    glDisableVertexAttribArray(glGetAttribLocation(programID, "aPosition"));
    glDisableVertexAttribArray(glGetAttribLocation(programID, "aTexcoord0"));
...
- (void)setupProgramVertex:(NSString *)v Fragment:(NSString *)f
{
    ...
    NSLog(@"GLRenderFaceUBeautify do setupProgram shader id = %d", programID);
//    glUseProgram(programID);
