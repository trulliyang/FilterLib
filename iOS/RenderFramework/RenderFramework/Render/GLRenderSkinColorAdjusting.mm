...
@interface GLRenderSkinColorAdjusting() {
@public
    GLuint programID;
    GLuint textureID;
    GLuint vtxBufID;
...
@implementation GLRenderSkinColorAdjusting
...
- (void)prepareRender
{
    if (0 == textureID)
    {
        NSLog(@"GLRenderSkinColorAdjusting do prepareRender no texture to render");
        return;
    }
...
- (void)setupProgramVertex:(NSString *)v Fragment:(NSString *)f
{
    ...
    NSLog(@"GLRenderSkinColorAdjusting do setupProgram shader id = %d", programID);
//    glUseProgram(programID);
